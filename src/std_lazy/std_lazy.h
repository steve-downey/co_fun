////////////////////////////////////////////////////////////////
// Reference implementation of std::lazy proposal D2506R0.
//
...
using _Rebind =
    typename allocator_traits<_Alloc>::template rebind_alloc<_Aligned_block>;
...
class _Promise_allocator { // statically specified allocator type
  ...
  public:
    static void*
    operator new(const size_t _Size) requires default_initializable<_Alloc> {
    ...
    static void* operator new(const size_t _Size,
                              ...
                              _Alloc2&& _Al,
                              _Args&...) {
        return _Allocate(static_cast<_Alloc>(static_cast<_Allocator>(
                             ::std::forward<_Alloc2>(_Al))),
                         _Size);
    }
    ...
    static void operator delete(void* const  _Ptr,
...
class _Promise_allocator<void> { // type-erased allocator
    ...
    static void* _Allocate(_Alloc2 _Al2, size_t _Size) {
        ...
        if constexpr (default_initializable<_Alloc> &&
            ...
            const _Dealloc_fn _Dealloc = [](void* const  _Ptr,
                                            const size_t _Size) {
                _Alloc       _Al{};
                const size_t _Count = (_Size + sizeof(_Dealloc_fn) +
                                       sizeof(_Aligned_block) - 1) /
            ...
            const _Dealloc_fn _Dealloc = [](void* const _Ptr, size_t _Size) {
                ...
                const size_t _Count = (_Size + sizeof(_Al) + _Align - 1) /
                                      sizeof(_Aligned_block);
                _Al.deallocate(static_cast<_Aligned_block*>(_Ptr), _Count);
            };
  ...
  public:
    ...
    static void operator delete(void* const  _Ptr,
        ...
        ::memcpy(&_Dealloc,
                 static_cast<const char*>(_Ptr) + _Size,
                 sizeof(_Dealloc_fn));
        _Dealloc(_Ptr, _Size);
    }
};
...
template <class _Ty>
concept _Await_suspend_result = same_as<_Ty, void> || same_as<_Ty, bool> ||
    __is_specialization_of<_Ty, coroutine_handle>;

template <class _Ty, class _Promise = void>
concept simple_awaitable = requires(_Ty&                              _Val,
                                    const coroutine_handle<_Promise>& _Coro) {
    { _Val.await_ready() } -> convertible_to<bool>;
    { _Val.await_suspend(_Coro) } -> _Await_suspend_result;
    _Val.await_resume();
};
...
template <class _Ty, class _Promise = void>
concept awaitable = _Has_member_co_await<_Ty, _Promise> ||
    _Has_ADL_co_await<_Ty, _Promise> || simple_awaitable<_Ty&, _Promise>;
...
class _Lazy_promise_base {
  public:
    ...
    _Lazy_promise_base(_Lazy_promise_base&& _That) noexcept(
        ...
        case _Discriminator::_Data:
            ::std::construct_at(::std::addressof(_Data),
                                ::std::move(_That._Data));
            ::std::destroy_at(::std::addressof(_That._Data_));
            break;
        case _Discriminator::_Exception:
            ::std::construct_at(::std::addressof(_Except),
                                ::std::move(_That._Except));
  ...
  private:
    ...
    struct _Final_awaiter {
        ...
        [[nodiscard]] coroutine_handle<>
        await_suspend(coroutine_handle<_Promise> _Coro) noexcept {
#ifdef __cpp_lib_is_pointer_interconvertible
            static_assert(
                is_pointer_interconvertible_base_of_v<_Lazy_promise_base,
                                                      _Promise>);
    ...
    struct _Awaiter {
        ...
        [[nodiscard]] bool await_ready() noexcept { return !_Coro; }
...
class [[nodiscard]] lazy {
  public:
    static_assert(
        is_void_v<_Ty> || is_reference_v<_Ty> ||
    ...
    struct _EMPTY_BASES promise_type : _Promise_allocator<_Allocator>,
                                       _Lazy_promise_base<_Ty> {
    ...
    [[nodiscard]] typename _Lazy_promise_base<_Ty>::_Awaiter
    operator co_await() {
        // Pre: _Coro refers to a coroutine suspended at its initial suspend
        // point
        assert(_Coro && !_Coro.done() &&
               "co_await requires the lazy object to be associated with a "
               "coroutine "
               "suspended at its initial suspend point");

        auto& _Promise_base =
            static_cast<_Lazy_promise_base<_Ty>&>(_Coro.promise());
        return typename _Lazy_promise_base<_Ty>::_Awaiter{
            coroutine_handle<_Lazy_promise_base<_Ty>>::from_promise(
                _Promise_base)};
    }
    ...
    [[nodiscard]] _Ty sync_await() {
        ...
        // point
        assert(_Coro && !_Coro.done() &&
               "sync_await requires the lazy object to be associated with a "
               "coroutine "
               "suspended at its initial suspend point");

        simple_awaitable<noop_coroutine_handle> auto _Simple =
                                                     operator co_await();
